...
- 🔐 **双向协议支持**，完整实现充电桩上报与平台下发双向报文解析
- 🎭 **智能模拟支持**，内置TCP模拟器，支持自动化充电流程仿真测试
- 🔄 **高效协议解析**，零框架依赖，字节数组直接转换，无冗余开销
- 🎯 **高效指令生成**，提供全平台控制指令构建能力，一键生成标准下发报文
- 🛠️ **数据安全校验**，内置CRC16/MODBUS等多种校验算法
- 🔗 **简易接口开发**，接口极简，仅负责字节数组与对象转换，网络通信完全解耦，适配各种业务场景
- 🔧 **灵活属性配置**，支持日志开关动态配置，适应多种运行环境
...
@Configuration
public class KCloudChargeConfig {
     ...
     * <p>
     * 初始化TCP设备模拟器
     * 项目启动自动连接，项目停止自动关闭连接.可配置多个设备
     *
     */
    @Bean(initMethod = "start", destroyMethod = "stop")
    public DeviceSimulator deviceSimulator() {
        StandardDevice standardDevice1 = new StandardDevice()
                .setDeviceId("52010600109041")
                .setDeviceType(1)
                .setGunNum(1)
                .setProtocolVersion("1.6")
                .setProgramVersion("DLF:1218")
                .setNetworkLinkType(2)
                .setSimNo("89860222152530123307")
                .setCarrier(0);
        StandardDevice standardDevice2 = new StandardDevice()
                .setDeviceId("52010600109042")
                .setDeviceType(1)
                .setGunNum(2)
                .setProtocolVersion("1.6")
                .setProgramVersion("DLF:1219")
                .setNetworkLinkType(1)
                .setSimNo("89860222152530123308")
                .setCarrier(2);

        List<StandardDevice> deviceList = new ArrayList<StandardDevice>();
        deviceList.add(standardDevice1);
        deviceList.add(standardDevice2);
        // 创建设备模拟器
        return DeviceSimulator.create(
                SimulatorConfig.builder()
                        .serverIP("127.0.0.1")
                        .serverPort(9001)
                        .simulatorDeviceList(deviceList)
                        .build()
        );
    }
}
...
| analysisHeartbeatLogOutput  | boolean | true | 解析器-心跳日志开关 |
