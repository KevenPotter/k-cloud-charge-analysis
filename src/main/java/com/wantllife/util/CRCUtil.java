 ...
 *
 * @author KevenPotter
 * @date 2026-04-21 10:52:01
 */
public class CRCUtil {

    /**
     * CRC高字节查表数组(gabyCRCHi = CRC High Byte Table)
     * 含义:云快充协议官方定义的 CRC16高8位常量对照表
     * 作用:配合算法快速计算CRC高字节,无需实时运算,提升效率
     */
    ...
    /**
     * CRC低字节查表数组(gabyCRCLo = CRC Low Byte Table)
     * 含义:云快充协议官方定义的CRC16低8位常量对照表
     * 作用:配合算法快速计算CRC低字节,固定常量,不可修改
     */
    private static final byte[] gabyCRCLo = {
     ...
     * 算法:查表法 CRC16
     *
     * @param data 待计算CRC的数据体(序列号+加密标志+帧类型+业务数据)
     * @return 原始16位CRC整数(高字节 + 低字节)
     * @author KevenPotter
     * @date 2026-04-21 10:50:32
     */
    private static int calculateRawCRC(byte[] data) {
        // CRC初始值(官方固定:0xFF 0xFF)
        int byCRCHi = 0xff;
        int byCRCLo = 0xff;
        // 查表索引
        int byIdx;
        ...
        for (byte b : data) {
            // 计算查表索引:高字节 ^ 当前数据
            byIdx = (byCRCHi ^ (b & 0xFF)) & 0xFF;
            // 更新高字节:低字节 ^ 查表高字节
            byCRCHi = (byCRCLo ^ (gabyCRCHi[byIdx] & 0xFF)) & 0xFF;
            // 更新低字节:直接取查表低字节
     ...
     *
     * @param data         待校验字节数组
     * @param littleEndian true:小端序(低字节在前) false:大端序(高字节在前，设备实际使用)
     * @return 4位十六进制CRC字符串
     * @author KevenPotter
     * @date 2026-04-21 10:45:17
     */
    public static String getYKCCRC(byte[] data, boolean littleEndian) {
        int rawCrc = calculateRawCRC(data);
        if (littleEndian) {
            // 小端:低字节在前
            byte low = (byte) (rawCrc & 0xFF);
            byte high = (byte) (rawCrc >> 8);
            return String.format("%02X%02X", low, high);
        } else {
            // 大端:高字节在前
            return String.format("%04X", rawCrc);
        }
    }
     ...
     * @date 2026-04-21 10:45:17
     */
    public static String getYKCCRC(byte[] data) {
        return getYKCCRC(data, false);
    }
}