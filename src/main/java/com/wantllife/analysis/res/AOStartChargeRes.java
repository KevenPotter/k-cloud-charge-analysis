 ...
 *
 * @author KevenPotter
 * @date 2026-04-27 11:11:02
 */
@Slf4j
@Data
@Accessors(chain = true)
@EqualsAndHashCode(callSuper = true)
public class AOStartChargeRes extends FrameHeader {
    ...
    public static byte[] buildCommand(String deviceId, Integer gunNo, String tradeNo, String logicalCardNo, String physicalCardNo, BigDecimal balance) {
        AOStartChargeRes res = new AOStartChargeRes();
        res.setSeqNo(RandomUtil.randomNumbers(4));
        res.setFrameType(DOWN_START_CHARGE);
    ...
    private void log(String rawHexMsg) {
        ...
        sb.append(String.format("🟠%s 远程控制启机  设备编号    deviceId                     : %s\n", devLabel, deviceId));
        sb.append(String.format("🟠%s 远程控制启机  枪口编号    gunNo                        : %s\n", devLabel, gunNo));
        sb.append(String.format("🟠%s 远程控制启机  交易编号    tradeNo                      : %s\n", devLabel, tradeNo));
        sb.append(String.format("🟠%s 远程控制启机  逻辑卡号    logicalCardNo                : %s\n", devLabel, logicalCardNo));
        sb.append(String.format("🟠%s 远程控制启机  物理卡号    physicalCardNo               : %s\n", devLabel, physicalCardNo));
        sb.append(String.format("🟠%s 远程控制启机  账户余额    balance                      : %s\n", devLabel, balance));
