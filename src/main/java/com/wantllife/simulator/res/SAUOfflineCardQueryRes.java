...
public class SAUOfflineCardQueryRes extends FrameHeader {
    ...
    private void log(String rawHexMsg) {
        ...
        log.info("🚀 【0x47】 {} 电卡查询应答  设备编号    deviceId                     : {}", PURPLE + deviceId + RESET, deviceId);

        for (int i = 0; i < queryResultList.size(); i++) {
            System.out.println();
            QueryResult queryResult = queryResultList.get(i);
            log.info("🚀 【0x47】 {} 电卡查询应答  第{}物号    physicalCardNo               : {}", PURPLE + deviceId + RESET, NumberChineseFormatter.format(i, false, false), queryResult.getPhysicalCardNo());
            log.info("🚀 【0x47】 {} 电卡查询应答  查询结果    queryResult                  : {}", PURPLE + deviceId + RESET, queryResult.getQueryResult() == 0 ? "卡不存在" : "卡存在");
        }
        System.out.println();
    }
}
