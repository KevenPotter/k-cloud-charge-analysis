...
public class SBARebootRes extends FrameHeader {
    ...
    private void log(String rawHexMsg) {
        log.info("-------------------------------------------------------------------------------------------");
        log.info("🚀 【0x91】 {} 远程重启应答  原始报文    rawMsg                       : {}", PURPLE + deviceId + RESET, rawHexMsg);
        log.info("🚀 【0x91】 {} 远程重启应答  设备编号    deviceId                     : {}", PURPLE + deviceId + RESET, deviceId);
        log.info("🚀 【0x91】 {} 远程重启应答  设置结果    setResult                    : {}", PURPLE + deviceId + RESET, setResult == 0 ? "重启失败" : "重启成功");
        System.out.println();
    }
}
